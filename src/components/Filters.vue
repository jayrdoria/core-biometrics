<template>
	<div class="filters-component">
		<div :class="isLabelVisible && 'flex items-center'">
			<div v-if="isLabelVisible" class="text-14 core-text-accent-1 q-mr-md">Filters:</div>
			<div class="flex justify-end">
				<q-select
					v-if="isSelectVisible"
					class="minmax-width core-border-radius-50 core-border-violet-0 core-select-field height-31 q-mr-md"
					dense
					borderless
					v-model="type"
				...
				<q-input
					...
					} core-border-radius-50 core-border-violet-0 core-input-field height-${fieldHeight} use-input q-px-md ${
					...
					><template v-slot:append>
						<q-icon
							...
							name="close"
							@click="fetchAll()"
							class="cursor-pointer core-text-violet-0"
						/>
						<q-icon
							@click="searchKeyword()"
							class="core-text-violet-0 cursor-pointer"
				...
				<q-select
					...
					option-value="value"
					option-label="label"
					:class="`minmax-width core-border-violet-0 core-border-radius-50 core-select-field height-31 text-14 ${
						isLabelVisible && 'q-mr-md'
					}`"
				...
				>
					<template v-slot:after-options>
						<q-separator class="core-text-violet-0" />
						<div v-if="isDateRangeVisible" class="q-px-sm">
							<q-item-section class="core-text-accent-2 q-my-sm">Custom Range</q-item-section>
							<q-form ref="formDateRange" greedy>
								<div>
									<label>From</label>
									<q-input
										class="
											core-border-violet-0 core-border-radius-5 core-input-field
											filter-date-field
											q-px-sm
											label-accent-2
										"
											...
											>
												...
												>
													<q-date
														...
														minimal
														flat
														class="core-bg-violet-0 text-white"
														color="white"
														text-color="black"
													/>
												</q-popup-proxy>
								...
								<div class="q-mt-xs">
									...
									<q-input
										class="
											filter-date-field
											core-border-radius-5 core-input-field core-border-violet-0
											label-accent-2
											q-px-sm
										"
										v-model="toDate"
						...
						<div v-else class="q-pa-sm">
							...
							>
								<template v-slot:append>
									<q-icon
										name="calendar_today"
										class="cursor-pointer margin-top-n13 core-text-accent-2 text-14"
									>
										<q-popup-proxy
											ref="selectDate"
											cover
	...
	setup(props) {
		...
		const filter = (action) => {
			...
			if (action === 'time') {
				...
				date_session.value !== null && (payload.search.date_session = date_session.value);
			} else {

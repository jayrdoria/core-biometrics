<template>
	<div class="filter">
		<div>
			<q-dialog v-model="FilterDialogState" position="bottom" persistent :class="DarkMode && 'core-dark-mode'">
				<q-card class="filter-card core-bg-primary core-text-primary">
					<q-card-section>
						<q-btn
							flat
							icon="arrow_back"
						...
						<div v-if="isSearchVisible" class="q-mt-xl">
							...
							<q-input
								class="core-border-accent-0 core-input-field full-width q-px-md q-mr-md"
								dense
								borderless
								v-model="keyword"
								placeholder="Search"
								><template v-slot:append>
									<q-icon
										@click="searchKeyword()"
										class="core-text-accent-0 cursor-pointer"
							...
							<div class="date q-mt-sm">
								<q-input dense borderless :model-value="dateRange ? dateRange.from : dateRange">
									<template v-slot:append>
										<q-icon
											name="event"
											:class="calendar ? 'core-text-accent-0' : 'core-text-accent-2'"
											@click="calendar = true"
										/>
									</template>
								</q-input>
					...
					<div v-if="calendar" class="calendar">
						...
						<q-date
							class="full-width no-shadow core-bg-primary"
							mask="MMM DD, YYYY"
							v-model="dateRange"
...
import { FilterDialogState, ToggleFilterDialogState } from '../composables/Triggers';

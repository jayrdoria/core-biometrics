<template>
	<div class="user-delete width-300">
		<div class="full-width flex justify-center">
			<div class="core-border-radius-10 bg-white width-auto q-pa-sm">
				<q-icon name="iconfont icon-delete-fill" class="core-text-accent-0" size="30px" />
			</div>
		</div>
		...
		<q-input
			...
			class="core-input-field standard dialog-input core-border-radius-10 q-mt-sm q-mb-md"
		/>
		<q-btn
			@click="deleteUser()"
			flat
			...
			label="Proceed"
			dense
			:class="`core-bg-accent-0 full-width q-py-sm q-mb-s core-border-radius-${
...
import { DeleteUser, UserDelete, ValidatePassword } from '../../../composables/UserManagement';
export default {
	setup() {
		const $q = useQuasar();
				...
				if (response.status === 'success') {
					...
					DeleteUser({ id: UserDelete.value.id }).then((response) => {
						...
						$q.notify({
							...
							classes: `${
								status ? 'core-success-notif' : 'core-error-notif'

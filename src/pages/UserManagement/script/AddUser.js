...
import { FetchUserDetails, InsertUser, UpdateUser, ValidateEmail } from '../../../composables/UserManagement';
...
export default {
	setup() {
			...
			FetchUserDetails({ id: route.params.id }).then((response) => {
				if (response.status === 'success' && response.data.length) {
					form.value = {
						...
						supervisor_id: response.data[0].supervisor_id ? response.data[0].supervisor_id : null,
					};
		...
		const levelRule = [(val) => (val !== null && val !== '') || '', (val) => val >= 1 || ''];
		...
		const addUser = () => {
			userForm.value.validate().then((success) => {
				if (success) {
					...
					// Post request
					if (!route.params.id) {
						ValidateEmail({ email_address: form.value.email_address }).then((response) => {
